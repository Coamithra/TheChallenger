...
**`CHALLENGER_DISPLAY_EDIT` moves the editor into the display hook.** The report is then drawn in place of the draft — followed by a `stash()` link to the original, since replacing the draft outright leaves no placeholder to carry one — and the Stop hook only allows (phase `delivered`), or asks the questions the editor parked for it (phase `ask_pending`). It requires the `MessageDisplay` entry's `timeout` raised to 360 — the platform default is 10s (`Ei=1e4` in the 2.1.247 build) and a per-hook `timeout` overrides it. Nothing at display time identifies the turn's last message, so every message over `MIN_CHARS` is edited; `MIDTURN_NOTE` warns the editor about that, and `check_fidelity()` is the mechanical remainder of the agent's fact-check, falling back to the placeholder path when the rewrite cites something the draft never did. Display-only and best-effort: the interactive TUI currently ignores `displayContent` upstream, and the first turn of a fresh session always fails open (no model in the transcript yet).
