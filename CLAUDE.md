...
**Hooks cannot replace a rendered response.** The original is already on screen by the time the Stop hook runs; the edited report arrives as a follow-up message. This is a platform constraint, not a choice. The optional `challenger_display_hook.py` (`MessageDisplay` event, registered separately) works around it prospectively: it buffers candidate drafts as they render and collapses long ones to a placeholder before they are shown, stashing the hidden text in `.claude/challenger-drafts/` inside the session's project and linking it from the placeholder — the message stream renders no raw HTML, so an inline foldout is not an option, and links only open when they resolve under the working directory (see plan.md item 21). The stash directory self-ignores with a `*` .gitignore, so it never reaches the enabled project's git status. It shares all config, gates, and state helpers by importing `challenger_hook`, and couples back through the `hidden_any` flag in `%TEMP%\challenger-display-<session_id>.json` — when set, an editor failure makes the Stop hook's `fail_open()` block once to have the agent repost the hidden draft instead of silently allowing.

**The editor runs in the display hook.** The report is drawn in place of the draft — followed by a `stash()` link to the original, since replacing the draft outright leaves no placeholder to carry one — and the Stop hook only allows (phase `delivered`), or asks the questions the editor parked for it (phase `ask_pending`). It requires the `MessageDisplay` entry's `timeout` set to 360 — the platform default is 10s (`Ei=1e4` in the 2.1.247 build) and a per-hook `timeout` overrides it. The Stop hook keeps its own editing path for the sessions this cannot reach: no companion registered, a client that drops `displayContent`, or a rewrite `check_fidelity()` handed back. Nothing at display time identifies the turn's last message, so every message over `MIN_CHARS` is edited; `MIDTURN_NOTE` warns the editor about that, and `check_fidelity()` is the mechanical remainder of the agent's fact-check, falling back to the placeholder path when the rewrite cites something the draft never did. Display-only and best-effort: the interactive TUI currently ignores `displayContent` upstream, and the first turn of a fresh session always fails open (no model in the transcript yet).
